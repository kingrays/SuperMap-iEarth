...
import { ILayerPanelConfigItem } from "@/views/panelLayer/index.d";

const layerServiceData = window.layerServiceData;

interface ILayerStoreState {
  layerPanelTreeData: ILayerPanelConfigItem[];
  layerServiceData: any;
  layerTreeData: any[];
}

export const useLayerStore = defineStore("useLayerStore", {
  state: (): ILayerStoreState => ({
    // 添加数据所用到的三类在线服务
    layerPanelTreeData: [],
  ...
  getters: {},
  actions: {
    setLayerPanelTreeData(val: ILayerPanelConfigItem[]) {
      this.layerPanelTreeData = val;
    },
    // 更新图层
    ...
    updateS3M() {
      ...
      // 扁平化集合
      let s3mOptionList_flatten: any =
        this.flattenS3MCollections(s3mOptionList);
      ...
      for (let i = 0; i < viewer.scene.layers._layerQueue.length; i++) {
        ...
        // 只对不存在于图层列表中的图层做处理
        let target = s3mOptionList_flatten.find(
          (item) => item.id == S3Mlayer.name
        );
        if (!target) {
    ...
    updateMVT() {
      ...
      if (
        mvtMapFirst &&
        mvtMapFirst.mapboxStyle &&
        mvtMapFirst.mapboxStyle.layers.length > 0
      ) {
        // MVT底层styleLayer共用,所以只需要第一个MVT图层即可获取所有的子图层
        mvtMapFirst.mapboxStyle.layers.forEach(
          (styleLayer: any, index: string) => {
            if (styleLayer.source) {
              if (!styleLayer.customName) styleLayer.customName = styleLayer.id;

              // 计算显隐
              const visibleValue = mvtMapFirst.getLayoutProperty(
                styleLayer.id,
                "visibility"
              );
    ...
    updateTIN() {
      ...
      if (!viewer.terrainProvider.customName)
        viewer.terrainProvider.customName = terrainLayerName;
      this.layerTreeData[3].children.push({
        ...
        children: undefined,
        isShow:
          viewer.terrainProvider._baseUrl || viewer.terrainProvider._urls
            ? true
            : false,
    ...
    getImageryLayerName(imageryLayer: any) {
      ...
      let imageryProvider =
        imageryLayer.imageryProvider || imageryLayer._imageryProvider;
      ...
      if (imageryProvider instanceof SuperMap3D.SuperMapImageryProvider) {
        ...
        imageryProvider instanceof SuperMap3D.CGCS2000MapServerImageryProvider
      ) {
        ...
        imageryProvider instanceof SuperMap3D.BingMapsImageryProvider
      ) {
        return $t("BingMap");
      } else if (
        imageryProvider instanceof SuperMap3D.TiandituImageryProvider
      ) {
        return $t("TIANDITU");
      } else if (
        imageryProvider instanceof SuperMap3D.UrlTemplateImageryProvider
      ) {
        let imgLayerUrl =
          imageryProvider.url ||
          imageryProvider._url ||
          imageryProvider.tablename;
        return imgLayerUrl; // TODO:精细化处理
      } else if (
        imageryProvider instanceof SuperMap3D.SingleTileImageryProvider
      ) {
        ...
        imageryProvider instanceof SuperMap3D.TileCoordinatesImageryProvider
      ) {
        return $t("lnglatMap");
      }
      ...
      if (window.iEarthConsole)
        console.log(
          "未能通过imageryProvider类型计算出影像图层名称,使用之前的代码计算"
        );

      let imageUrl =
        imageryLayer._imageryProvider.url || imageryLayer._imageryProvider._url;
      ...
      // 项目底图
      let targetItem = this.layerServiceData.onlineBaseLayerList.find(
        (item: any) => item.url === imageUrl
      );
      if (targetItem) {
        if (targetItem.name) {
      ...
      if (imageUrl && imageUrl.indexOf("realspace/datas/") != -1) {
        let otherImageLayerName = imageUrl
          .split("realspace/datas/")[1]
          .replace("/", "");
    ...
    getTerrainLayerName(): any {
      ...
      if (window.iEarthConsole)
        console.log(
          "未能通过terrainProvider类型计算出地形图层名称,使用之前的代码计算"
        );
      ...
      if (terrainProvider._baseUrl) {
        ...
        if (baseUrl.indexOf("3D-stk_terrain") != -1) {
          ...
          if (baseUrl.includes("info/data/path")) {
            ...
            // 之前遗留的
            return baseUrl
              .split("realspace/services/")[1]
              .split("/rest/realspace")[0];
          } else if (baseUrl.indexOf("iserver/services") != -1) {
            // 之前遗留的
            return baseUrl
              .split("iserver/services/")[1]
              .split("/rest/realspace")[0];
          } else {
            return;
          }
