...
function computedServiceChooseType(publicServiceList) {
  ...
  viewer.scene.layers._layerQueue.forEach((s3mLayer) => {
    ...
    if (typeof s3mBaseUriPath != "string") {
      // 11.3和主版本这里不一致，用于兼容主版本
  ...
  viewer.imageryLayers._layers.forEach((imageLayer) => {
    const imageUrl =
      imageLayer._imageryProvider._baseUrl || imageLayer._imageryProvider.url;
    if (!imageUrl) return;
    publicServiceList.forEach((item) => {
  ...
  viewer.scene._vectorTileMaps._layerQueue.forEach((mvtLayer) => {
    const mvtUrl = mvtLayer._provider
      ? mvtLayer._provider.tablename
      : mvtLayer._url;
    if (!mvtUrl) return;
    publicServiceList.forEach((item) => {
      ...
      if (sceneUrl.includes("/restjsr/v1/vectortile")) {
        let subString = sceneUrl
          .split("/restjsr/v1/vectortile")[0]
          .split("/")
          .pop();
...
function addScene(url: string, sceneName: string, type: any) {
  if (checkURL(url)) {
    ...
    if (sceneName === "transformerStation") {
      ...
      let promise = window.viewer.scene.open(url, undefined, {
        autoSetView: true,
      });
      SuperMap3D.when.all(promise, function (layers: any) {
        layers.forEach((layer: any) => {
          layer.residentRootTile =
            window.customConfig && window.customConfig.s3mLayer_residentRootTile
              ? true
              : false;
          ...
          layer.minTransparentAlpha =
            window.customConfig.minTransparentAlpha || 0.1; // 默认值为0.1
...
function addCBD(url: string) {
  ...
  let L00 = new SuperMap3D.Cartesian3(
    0.255985647439957,
    0.324294656515121,
    0.448104858398438
  );
  let L1_1 = new SuperMap3D.Cartesian3(
    0.052135497331619,
    0.127489775419235,
    0.259717047214508
  );
  let L10 = new SuperMap3D.Cartesian3(
    -0.043244555592537,
    -0.037950038909912,
    -0.036239303648472
  );
  let L11 = new SuperMap3D.Cartesian3(
    0.014937655068934,
    -0.003836219897494,
    -0.021041290834546
  );
  let L2_2 = new SuperMap3D.Cartesian3(
    0.03790882602334,
    0.013326642103493,
    -0.008756417781115
  );
  let L2_1 = new SuperMap3D.Cartesian3(
    -0.040351137518883,
    -0.020264262333512,
    -0.004807807970792
  );
  let L20 = new SuperMap3D.Cartesian3(
    0.004116172902286,
    0.001403471920639,
    -0.004473014734685
  );
  let L21 = new SuperMap3D.Cartesian3(
    -0.039947938174009,
    -0.028241466730833,
    -0.01187295652926
  );
  let L22 = new SuperMap3D.Cartesian3(
    0.042825646698475,
    0.035332202911377,
    0.01450318377465
  );
  scene.specularEnvironmentMaps =
    "./Resource/CBD/HongKong_sphere_original_1k.ktx2";
    ...
    SuperMap3D.when(
      ...
      function (layers) {
        ...
        if (layer5 && layer5 instanceof SuperMap3D.S3MTilesLayer) {
          ...
          layer5.waterColor = new SuperMap3D.Color(
            0 / 255,
            53 / 255,
            43 / 255,
            1
...
function addBDZ(url: string) {
  ...
  function loadLight(lightChecked) {
    if (lightChecked) {
      ...
      scene.specularEnvironmentMaps =
        "./Resource/BDZ/drakensberg_solitary_mountain_1k_2_S-20.hdr";
